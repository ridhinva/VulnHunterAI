...
@dataclass
class Finding:
    id: str=""; title: str=""; severity: str="info"; category: str=""
    target: str=""; description: str=""; evidence: list=None
    proof_of_concept: str=""; remediation: str=""; cvss_score: float=0.0
    cvss_vector: str=""; references: list=None; tool: str=""
    timestamp: float=0; false_positive: bool=False; confirmed: bool=False
    def __post_init__(self):
        if self.evidence is None: self.evidence = []
        if self.references is None: self.references = []
...
class FindingsDB:
    ...
    CREATE TABLE IF NOT EXISTS findings (
        ...
        remediation TEXT, cvss_score REAL, cvss_vector TEXT, references TEXT,
    ...
    async def _db_conn(self):
        if self._db is None:
            ...
            await self._db.executescript(self.SCHEMA); await self._db.commit()
        return self._db
    async def add(self, f: Finding) -> str:
        ...
        await db.execute(
            ...
            (f.id, f.title, f.severity, f.category, f.target, f.description,
             ...
             f.cvss_score, f.cvss_vector, json.dumps(f.references), f.tool,
             f.timestamp, int(f.false_positive), int(f.confirmed)))
        await db.commit(); return f.id
    async def list_all(self, severity=None, category=None, limit=1000):
        db = await self._db_conn()
        q = "SELECT * FROM findings WHERE 1=1"; params = []
    ...
    async def count(self, severity=None):
        db = await self._db_conn()
        q = "SELECT COUNT(*) FROM findings"; params = []
    ...
    async def export(self, fmt="json"):
        ...
        if fmt == "json":
            with open(path, "w") as f: json.dump([fd.to_dict() for fd in findings], f, indent=2, default=str)
        return path
    ...
    def _row_to_finding(self, r):
        return Finding(id=r[0],title=r[1],severity=r[2],category=r[3],target=r[4],
            ...
            references=json.loads(r[11]),tool=r[12],timestamp=r[13],
